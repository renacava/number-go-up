...
(defun percentage (x total)
  "Returns what percentage x is of total."
  (* 100.0 (/ x total)))

(defun percent-of (percent total)
  "Returns the number == percent of total, eg (percent 10 50) => 5"
  (* percent (/ total 100)))

(defun round-to-nearest (x interval)
  "Returns x rounded to the nearest interval. May return 0."
  (multiple-value-bind (n remainder) (floor x interval)
    (if (> remainder (half interval))
        (+ interval (* interval n))
        (* interval n))))

(let ((alphanumeric-chars "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ0123456789"))

  (defun leave-only-alphanumerics (string)
   (coerce
    (loop for char across string
          when (position char alphanumeric-chars)
          collect char)
    'string)))

(let ((ascii-chars #(#\  #\! #\" #\# #\$ #\% #\& #\' #\( #\) #\* #\+ #\, #\- #\. #\/ #\0 #\1 #\2
  ...
  (defun strip-non-ascii (string)
    (coerce
     (loop for char across string
           when (position char ascii-chars)
           collect char)
     'string)))

(defun get-real-time-seconds ()
  "Returns the current time in seconds since some arbitrary point in time, as a positive real number. Use this to measure time passage, by calling it at different points in your code. The only use of this function is to measure passage of time between calls to this function, the actual return value is arbitrary, the difference is what's important."
  (float (/ (get-internal-real-time) internal-time-units-per-second)))

(defun count-words (string)
  "Returns the number of words in the given string."
  (length (tokenise string #\ )))

(defun count-sentences (string)
  "Counts the number of periods in the given string."
  (1- (length (tokenise string #\.))))

(defun count-alphanumeric-characters (string)
  "Returns the number of letters and numbers in string."
...
(defun average-words-per-sentence (sentences)
  "Returns the average number of words per sentence in sentences."
  (let ((words (reduce #'+ (mapcar #'count-words sentences))))
    (/ words (length sentences))))

(defun average-word-length (text)
  "Returns the average length of words in the given text."
  (/ (count-alphanumeric-characters text) (count-words text)))

(defun get-sentences (text)
  "Returns a list of sentences from the given text."
  (let ((sentences (remove-if (lambda (sentence) (equal sentence "")) (tokenise text #\.))))
    (loop for sentence in sentences
          collect (concatenate 'string
                               (if (equal (aref sentence 0) #\ )
                                   (subseq sentence 1)
                                   sentence)
                               "."))))

(defun string-empty? (string)
  "Returns T when the given string doesn't contain any alphanumeric characters."
  (let ((string (string-downcase string)))
    (loop for char across "abcdefghijklmnopqrstuvwxyz0123456789"
          when (position char string)
          do (return-from string-empty? nil))
    t))
(defun month-to-num (month-string)
  "Returns a number according to the index of the month-string in the year, eg June => 6"
  (let ((month-string (string-downcase (subseq month-string 0 3))))
    (cond ((equal month-string "jan") 1)
          ((equal month-string "feb") 2)
          ((equal month-string "mar") 3)
          ((equal month-string "apr") 4)
          ((equal month-string "may") 5)
          ((equal month-string "jun") 6)
          ((equal month-string "jul") 7)
          ((equal month-string "aug") 8)
          ((equal month-string "sep") 9)
          ((equal month-string "oct") 10)
          ((equal month-string "nov") 11)
          ((equal month-string "dec") 12)
          (t 0))))
